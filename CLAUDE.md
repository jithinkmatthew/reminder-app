...
- Manual test checklist after changes:
  ...
  10. Delete (×) a task — confirm a browser `confirm()` dialog appears first; canceling it leaves the task in place. Confirming while the task is active should stop the timer (worker stopped, wake lock released), not just remove it from the list and timeline.
  ...
  13. Click the ✎ edit icon on a non-active task — the modal should open pre-filled with that task's name/time/duration/interval, titled "Edit task" with a "Save changes" button; saving should update the task in place (list and timeline) rather than creating a duplicate. Confirm the ✎ icon is not shown at all while a task is running or paused.
  14. Open the modal and confirm the only way to close it is the × in the header — clicking the dark overlay outside the card should do nothing.
  15. Reload "Today's schedule" and confirm it opens scrolled so the current hour's row is at the top (not 12AM), and that a red line marks the current time and advances as time passes (e.g. visible after leaving a timer running for a minute).
...
- **Day timeline** (`renderDayTimeline()`): renders a 24-hour vertical schedule — an hour-label sidebar plus a track where each task with a `scheduledTime` is drawn as an absolutely-positioned block, `top`/`height` computed from its start time and total duration (`durationMin + durationSec/60 + extendedMin`, `TIMELINE_HOUR_HEIGHT` px per hour). Overlapping tasks are assigned side-by-side columns via a greedy interval-scheduling pass (`columnEnds`). Blocks get `.active`/`.running`/`.done` styling mirroring the task card's state. A completed task's block additionally renders a two-segment status bar (green = on-time portion, orange = extra-time portion, sized by `extendedMin`'s share of total time taken).
  - **Scroll-to-now on load**: `.day-timeline` is a scrollable container (`max-height: 560px`) that otherwise always starts scrolled to 12AM at the top. A module-level `hasScrolledTimelineToNow` flag makes `renderDayTimeline()` set `timeline.scrollTop` to the current hour (`now.getHours() * TIMELINE_HOUR_HEIGHT`) the *first* time it runs, so the schedule opens with the current hour's row at the top instead of midnight. It intentionally only fires once — `renderDayTimeline()` re-runs on every `renderTasks()` call (including once a second while a timer is active), and re-scrolling on every render would fight the user's manual scrolling.
  - **Live "now" line**: a thin red `.timeline-now-line` div (with a small dot at the left edge) is appended into `.timeline-track` on every render, positioned by `top` from the current `getHours()*60 + getMinutes()` converted to px. Since it's rebuilt from scratch each render (the timeline's `innerHTML` is cleared at the top of the function) it naturally stays in sync with the clock as `renderTasks()` re-renders over time — no separate interval/timer needed for it.
- **Click-to-create/edit modal** (`openTaskModal(scheduledTime, taskToEdit)` / `closeTaskModal()` / `#taskModalOverlay`): the only way to add *or* edit a task. Opened four ways — clicking an hour label in the timeline sidebar, clicking a spot on the timeline track (`Math.floor` of the click's Y-position into an hour), the "+ Add new task" button under Settings (blank start time), or a task card's ✎ **edit icon** (pre-fills all fields from that task, including duration split back into hours/minutes). Passing a `taskToEdit` switches the modal into edit mode: title becomes "Edit task", the submit button becomes "Save changes", and module-level `editingTaskId` tracks which task is being edited. Fields: task name, start time, duration as **Hours + Minutes** (no seconds in the UI — `durationSec` is always saved as `0`), and alert interval. `#modalAddBtn`'s handler validates a non-empty name and non-zero total duration, then either mutates the existing task in place (`editingTaskId` set) or calls `addTask(...)` (create mode). The modal's only close affordance is the `#modalCloseBtn` (×) button in the header — there is deliberately no click-outside-to-close and no Cancel button; don't reintroduce either without being asked.
...
- **Editing**: each task card has a ✎ icon button (`.task-edit`) between the pause/stop icon and the × delete icon, opening the modal pre-filled via `openTaskModal(task.scheduledTime, task)`. It's **hidden (`display: none`) whenever the task is active** (`isActive` — covers both running and paused) — a task's schedule/duration/name can't be edited while it's in progress.
- **Deleting**: the × icon on a task card now calls `confirm("Are you sure you want to delete this task?")` before removing it; canceling the browser dialog leaves the task untouched. This is a native blocking `confirm()`, not a custom modal — keep it that way unless asked to build a styled confirmation.
...
3. **Task list** (`#taskList`) — each task card:
   - Row 1 (single line): task name, then icon buttons — ▶ Play/Resume, a **merged pause/stop icon** (❙❙ while running → ■ once paused), ✎ Edit (hidden while the task is active), and × Close/Delete (asks for confirmation via `confirm()` before removing).
...
**"Today's schedule" card**: a scrollable 24-hour vertical timeline (`#dayTimeline`) — an hour-label sidebar (click a label to open the create-modal at that hour) plus a track where task blocks are positioned/sized by start time and duration, side-by-side when overlapping. Opens auto-scrolled to the current hour, and shows a live red line marking the current time.

**Click-to-create/edit modal** (`#taskModalOverlay`, hidden until opened): header with title (`#modalTitle`, "New task" or "Edit task") and a `#modalCloseBtn` (×) — the only way to close it. Fields: name, start time, duration (Hours + Minutes fields), alert interval, then a single submit button (`#modalAddBtn`, "Add task" or "Save changes").
